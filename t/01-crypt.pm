...
subtest "encrypt data" => sub {
    my $crypt = BOM::Utility::Crypt->new(keyname => 'test');
    ...
    my $decrypt = BOM::Utility::Crypt->new(keyname => 'test');
    my $data = $decrypt->decrypt_data(%$res);
    eq_or_diff $data, $test_data, "successfully restored data with correct password";
    $data = BOM::Utility::Crypt->new(keyname => 'fail_test')->decrypt_data(%$res);
...
subtest "encrypt payload" => sub {
    ...
    $data = BOM::Utility::Crypt->new(keyname => 'fail_test')->decrypt_payload(value => $cookie);
    ok !$data, "couldn't decrypt with the wrong password";
